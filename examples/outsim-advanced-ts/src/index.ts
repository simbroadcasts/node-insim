...
outSim.on('packet', (data: OutSimPack2) => {
  console.clear();
  console.log(`Velocity: ${data.OSMain.Vel[0]} ${data.OSMain.Vel[1]} ${data.OSMain.Vel[2]}`);
  console.log(`Airspeed: ${Math.sqrt(Math.pow(data.OSMain.Vel[0], 2) + Math.pow(data.OSMain.Vel[1], 2) + Math.pow(data.OSMain.Vel[2], 2))} m/s`);
  console.log(`Position: ${data.OSMain.Pos[0]} ${data.OSMain.Pos[1]} ${data.OSMain.Pos[2]}`);
